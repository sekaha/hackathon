...
FNT_SMALL = (os.path.dirname(os.path.abspath(__file__)) + "/m3x6.ttf", 16)
...
def get_delta(target_fps=None):
    if target_fps == None:
        target_fps = _game_speed

    reference_frame_time = 1 / target_fps
    return _true_game_speed / reference_frame_time
    ...
    for y in range(int(min_y), int(max_y + 1)):
        ...
        if len(intersections) >= 2:
            for k in range(0, len(intersections) - 1, 2):
                x_start, color_start = intersections[k]
                x_end, color_end = intersections[k + 1]
...
def _blit_buffer_canvas():
    ...
    _BLEND_SCREEN.surface.fill((0, 0, 0, 0)) 
...
def get_haxis(joystick):
    ...
    if len(_joysticks) > _joystick_environment:
        if joystick == JS_LSTICK:
            ...
            axis_val = _joysticks[_joystick_environment].get_axis(2)
        else:
